...
const PerformanceGraph: React.FC = () => {
  ...
  useEffect(() => {
    const processData = async () => {
      try {
        const trades = parseCSV(statementData);
        const sortedTrades = trades.sort(
          (a, b) => a.openDate.getTime() - b.openDate.getTime()
        );

        let cumulativeGain = 0;
        const performanceData = sortedTrades.map((trade) => {
          cumulativeGain += trade.gain;
          return {
            date: trade.openDate.toLocaleDateString("en-GB", {
              month: "short",
              day: "numeric",
              year: "2-digit",
            }),
        ...
        const processedData = performanceData.map((point, index) => {
          ...
          const sp500Value =
            (Math.pow(1 + dailySP500Return, dayIndex) - 1) * 100;
          ...
          return {
            date: point.date,
            deltaEdge: Number(point.value.toFixed(2)),
    ...
    datasets: [
      {
        ...
        data: chartData.map((d) => d.deltaEdge),
      ...
      },
      {
        ...
        data: chartData.map((d) => d.hedgeFund),
  ...
  const options = {
    ...
    scales: {
      ...
      y: {
        ...
        min: 0,
        max:
          Math.ceil(Math.max(...chartData.map((d) => d.deltaEdge)) / 10) * 10,
        beginAtZero: true,
      },
    },
    interaction: {
