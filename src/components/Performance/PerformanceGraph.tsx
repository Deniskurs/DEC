...
import { LuMessageCircleReply, LuZoomIn, LuArrowUpRight, LuArrowDownRight, LuMaximize, LuGalleryHorizontalEnd } from "react-icons/lu";
...
const PerformanceGraph: React.FC = () => {
  ...
  const [fullChartData, setFullChartData] = useState<ProcessedDataset | null>(null);
  const [options, setOptions] = useState<any>(null);
  const [error, setError] = useState<string | null>(null);
  const [activeDatasetIndex, setActiveDatasetIndex] = useState<number | null>(0); // Default to Delta Edge
  ...
  const generateInsights = useCallback((processedData: ChartDataPoint[]) => {
    ...
    const previousPoint = processedData[processedData.length - 2] || processedData[0];
    
    // Calculate Delta Edge performance
    const deltaEdgeValue = Number(latestPoint.deltaEdge.toFixed(2));
    const deltaEdgeTrend = latestPoint.deltaEdge > previousPoint.deltaEdge ? 'up' : 'down';
    
    // Calculate performance vs S&P 500
    const vsMarketValue = latestPoint.sp500 !== null 
    ...
    const avg = recentPoints.reduce((sum, val) => sum + val, 0) / recentPoints.length;
    ...
    const volatilityStatus = volatilityValue < 2 ? 'low' : volatilityValue < 5 ? 'moderate' : 'high';
  ...
  useEffect(() => {
    if (equityData && deadStrats) {
      ...
      try {
        ...
        setOptions(getChartOptions(isMobile, maxValue, minValue, activeDatasetIndex, isTablet));
        setError(null);
      } catch (err) {
  ...
  }, [equityData, isMobile, isTablet, deadStrats, activeDatasetIndex, dateRange, filterDataByDateRange, generateInsights]);
      ...
      <div className="relative p-6 sm:p-8 rounded-3xl bg-cream-50 shadow-lg border border-rich-blue-100/10">
        ...
        <div className="relative z-10 flex flex-wrap items-center justify-between gap-4 mb-8">
          <div className="relative flex items-center flex-wrap gap-1.5 bg-gradient-to-r from-white/80 to-white/60 p-1 rounded-lg shadow-sm border border-rich-blue-100/20 overflow-hidden group">
            ...
            <motion.div 
              className="absolute inset-0 bg-rich-blue-50/30 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
            />
                    ...
                    <div className="relative p-3 rounded-lg">
                      ...
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-blue-300 text-xs font-medium uppercase tracking-wider">Delta Edge</span>
                        </div>
                        <motion.div 
                      ...
                      <div className="flex items-baseline">
                        <span className="text-white text-2xl font-bold">{insights.deltaEdge.value}</span>
                        <span className="text-white/70 ml-1 text-lg">%</span>
                      </div>
                      
                      {/* Premium label */}
                      <div className="text-blue-300/80 text-xs mt-1">Current Return</div>
                      
                      {/* Bottom accent line */}
                      <div className={`absolute bottom-0 left-0 h-1 rounded-b-lg ${
                        insights.deltaEdge.trend === 'up' ? 'bg-blue-500/50' : 'bg-red-500/50'
                      }`} style={{ width: '30%' }}></div>
                    </div>
                  </div>
                    ...
                    <div className="relative p-3 rounded-lg">
                      ...
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-emerald-300 text-xs font-medium uppercase tracking-wider">vs S&P 500</span>
                        </div>
                        <motion.div 
                          className={`flex items-center justify-center h-6 w-6 rounded-full ${
                      ...
                      <div className="flex items-baseline">
                        <span className="text-white text-2xl font-bold">
                          {insights.vsMarket.value > 0 ? '+' : ''}{insights.vsMarket.value}
                        </span>
                        <span className="text-white/70 ml-1 text-lg">%</span>
                      </div>
                      
                      {/* Premium label */}
                      <div className="text-emerald-300/80 text-xs mt-1">Market Outperformance</div>
                      
                      {/* Bottom accent line */}
                      <div className={`absolute bottom-0 left-0 h-1 rounded-b-lg ${
                        insights.vsMarket.trend === 'up' ? 'bg-emerald-500/50' : 'bg-red-500/50'
                      }`} style={{ width: '30%' }}></div>
                    </div>
                  </div>
                    ...
                    <div className="relative p-3 rounded-lg">
                      ...
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-amber-300 text-xs font-medium uppercase tracking-wider">Volatility</span>
                        </div>
                        <motion.div 
                          className={`flex items-center justify-center h-6 w-6 rounded-full ${
                      ...
                      <div className="flex items-baseline">
                        <span className="text-white text-2xl font-bold capitalize">{insights.volatility.status}</span>
                        <span className="text-white/70 ml-2 text-sm">({insights.volatility.value})</span>
                      </div>
                      
                      {/* Premium description */}
                      <div className="text-amber-300/80 text-xs mt-1">Risk Assessment</div>
        ...
        <div className="relative z-10 mb-8">
          ...
          <div className={`relative z-10 flex flex-wrap items-center justify-center ${
            isMobile ? 'gap-1.5 overflow-x-auto py-3 px-2' : 'gap-3 sm:gap-4 py-4 px-4'
              ...
              >
                <div className="relative">
                  ...
                  >
                    <div className="flex items-center gap-2">
                      {/* Hand icon */}
                      <svg className="w-3.5 h-3.5 text-rich-blue-600" viewBox="0 0 24 24" fill="none">
                        <path d="M16.5 12V6.5C16.5 5.94772 16.0523 5.5 15.5 5.5C14.9477 5.5 14.5 5.94772 14.5 6.5V12" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
                        <path d="M14.5 10.5H11.5V6.5C11.5 5.94772 11.9477 5.5 12.5 5.5C13.0523 5.5 13.5 5.94772 13.5 6.5V7.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
                        <path d="M7.5 12V8C7.5 7.44772 7.94772 7 8.5 7C9.05228 7 9.5 7.44772 9.5 8V14.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
                        <path d="M9.5 14.5V11C9.5 10.4477 9.94772 10 10.5 10C11.0523 10 11.5 10.4477 11.5 11V14.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
                        <path d="M7.5 14.5V14.5C7.5 16.7091 9.29086 18.5 11.5 18.5H12.7576C13.533 18.5 14.2889 18.3297 14.9847 18" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
                      </svg>
                      
                      <span className="text-xs font-medium text-rich-blue-600">Click datasets to focus</span>
                    </div>
                  </motion.div>
                </div>
              </motion.div>
            ...
            {chartData &&
              chartData.datasets.map((dataset, index) => {
                ...
                const bgColor = typeof dataset.borderColor === 'string' ? dataset.borderColor : '#000';
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative flex items-center justify-center">
                      ...
                      <motion.div
                        className={`rounded-full ${isMobile ? 'w-3 h-3 mr-2' : 'w-3.5 h-3.5 mr-2.5'}`}
                    ...
                    <motion.span 
                      ...
                      transition={{ duration: 0.3 }}
                    >
                      {isMobile ? dataset.label.replace(' Returns', '').replace(' ETF', '').replace(' Index', '') : dataset.label}
                    </motion.span>
        ...
        <div className={`relative ${isMobile ? 'h-[350px]' : isTablet ? 'h-[400px]' : 'h-[450px]'}`}>
          {chartData && options && (
            <Line 
              ref={chartRef} 
              options={options} 
              data={{
                ...
                datasets: chartData.datasets.map((dataset, index) => {
                  ...
                  const isActive = activeDatasetIndex === null || activeDatasetIndex === index;
                  ...
                  if (isMobile) {
                    ...
                    return {
                      ...
                      borderWidth: isActive 
                        ? (dataset.borderWidth as number) * (activeDatasetIndex === index ? 1.2 : 1) // Less emphasis
                    ...
                    return {
                      ...
                      borderWidth: isActive 
                        ? (dataset.borderWidth as number) * (activeDatasetIndex === index ? 1.5 : 1) // Emphasize selected dataset
                      ...
                      pointRadius: isActive ? (index === activeDatasetIndex ? 3 : 0) : 0,
