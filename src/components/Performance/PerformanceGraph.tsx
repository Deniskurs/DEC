...
const PerformanceGraph: React.FC = () => {
  ...
  const {
    ...
    apiKey,
    startDate: equityData?.[0]?.date ?? "",
    baseValue: equityData?.[0]?.value ?? 0,
  });

  // Process and update chart data
  useEffect(() => {
    if (equityData && (!apiKey || (sp500Data && goldData && msciWorldData))) {
      try {
  ...
  }, [equityData, sp500Data, goldData, msciWorldData, isMobile, apiKey]);
  // Combined loading state
  const isLoading = equityDataLoading || (apiKey && marketDataLoading);
  ...
  return (
    <div className="relative w-full">
      <div className="relative p-8 rounded-3xl bg-gradient-to-br from-white/80 to-[#F8F7F3] backdrop-blur-xl shadow-2xl border border-rich-blue-100/20">
        ...
        {/* Error overlay */}
        {(error || marketDataError) && (
          <ErrorDisplay error={error || marketDataError || "Unknown error"} />
        )}
      </div>
    </div>
  );
